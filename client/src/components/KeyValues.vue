        ...
        <tr v-for="key in keys" :key="key">
            ...
            <td>
                ...
                <b
                    ...
                    :style="data[key] === highDuration && highDuration > 0 ? 'color: var(--int-red);' : ''"
                    >{{ format(key, data[key]) }}</b
                >
    ...
    computed: {
        keys() {
            ...
            return Object.keys(this.data).filter(
                (key) => key !== "preview" && key !== "lastModified" && key !== "tagger",
            )
        },
        innerHighDuration() {
            if (typeof this.data !== "object") return -1
            return Math.max(
    ...
    methods: {
        format(key: string, value: any) {
            ...
            if (key === "duration") {
                ...
                const G = 1000000000
                if (num < 1000) return num + "ns"
                if (num < 1000000) return num / 1000 + "us"
                if (num < G) return num / 1000000 + "ms"
                if (num < 60 * G) return num / G + "s"
                if (num < 3600 * G) return num / (60 * G) + "m"
                return num / (3600 * G) + "h"
            }
