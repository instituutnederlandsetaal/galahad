...
class FormatInducerTest {
    ...
    fun `Parse simple formats based on extension`() {
        ...
        assertEquals(
            ...
            DocumentFormat.fromFile(TestUtil.get("formats/shared/converter/karel_en_martijn.conllu"))
        )
        assertEquals(
            DocumentFormat.Txt,
    ...
    fun `Parse invalid formats`() {
        assertThrows(InvalidDocumentFormatException::class.java) { DocumentFormat.fromString("invalid") }
    }
    ...
    fun `Parse non-legacy XML formats`() {
        assertEquals(
            ...
            DocumentFormat.fromFile(TestUtil.get("formats/shared/converter/karel_en_martijn.folia.xml"))
        )
        assertEquals(
            DocumentFormat.TeiP5,
            DocumentFormat.fromFile(TestUtil.get("formats/shared/converter/karel_en_martijn.tei.xml"))
        )
        assertEquals(
            DocumentFormat.TeiP5,
        ...
        assertEquals(
            DocumentFormat.Naf,
            DocumentFormat.fromFile(TestUtil.get("formats/shared/converter/karel_en_martijn.naf.xml"))
        )
    }
