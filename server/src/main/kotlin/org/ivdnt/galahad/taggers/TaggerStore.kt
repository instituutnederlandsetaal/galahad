...
class TaggerStore : BaseFileSystemStore(
    ...
    val taggers: List<ExpensiveGettable<Tagger>>
        ...
        } // We can provide null since there is nothing like SOURCE_LAYER.yaml in ids

    fun getSummaryOrThrow(tagger: String, sourceLayerTagger: ExpensiveGettable<Tagger>? = null) =
        object : ExpensiveGettable<Tagger> {
            override fun expensiveGet(): Tagger {
                return getSummaryOrNull(tagger, sourceLayerTagger).expensiveGet()
                    ?: throw TaggerNotFoundException(tagger)
            }
        }
     ...
     */
    fun getSummaryOrNull(tagger: String, sourceLayerTagger: ExpensiveGettable<Tagger>?) =
        object : ExpensiveGettable<Tagger?> {
            override fun expensiveGet(): Tagger? {
