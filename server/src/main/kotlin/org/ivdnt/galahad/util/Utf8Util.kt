...
/**
 * A valid filename for windows and linux. Exceptions like COM1 still exist. Uses URL encoding.
 * Replaces invalid characters with a dash (-).
 *
    ...
    return buildString {
        for (char in this@escapeXML) {
            when {
                ...
                char.isLetterOrDigit() || char.isWhitespace() || (char.code in 0x20..0x7E) ->
                    append(char)
...
// Normally, periods (.) are allowed too, but they have a hierarchical significance, so add any
// periods yourself.
