...
private fun Node.isTextable(): Boolean {
    ...
    return when (this.tagName()) {
        ...
        null, "ref", "seg" -> this.parentNode?.isTextable() == true // null is for text nodes
        else -> this.tagName()?.startsWith("t-") == true
    }
}
...
class BLFXMLParser(
            ...
            DocumentFormat.TeiP4Legacy, DocumentFormat.TeiP5Legacy -> {
                ...
                posExtractor = {
                    ...
                    // But supports most cases
                        node ->
                    node.attributes.getNamedItem("pos")?.textContent
    ...
    private fun handleNode(node: Node) {
        if (node.nodeType == Node.ELEMENT_NODE) {
            ...
            for (child in oldChildNodes) {
                if (child.tagName() == "w" || child.tagName() == "pc") {
                    handleWordOrPunctNode(child); continue
                } // Word node handles itself, so don't recurse
                // Recursion
                handleNode(child)
                if (child.isTextable()) {
    ...
    private fun handleWordOrPunctNode(node: Node) {
        ...
        val needsSpacing =
            node.tagName() == "w" && plaintextTail.isNotBlank() && !Regex("""\s$""").containsMatchIn(plaintextTail)
