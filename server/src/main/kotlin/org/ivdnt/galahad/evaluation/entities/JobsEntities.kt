...
class JobsEntities(
    ...
    companion object {
        fun create(corpus: Corpus, jobsEvals: JobsEvaluations): JobsEntities {
            ...
            val docstddevs: Map<String, DocumentEntitiesStddev> = corpus.documents.readAllSequence().map{ it.name }.associateWith { doc ->
               ...
               val stddev = counts.mapValues { (_, values) ->
                ...
                DocumentEntitiesStddev(stddev, stddev.values.average())
            }
            ...
            return JobsEntities(jobEntities, jobstddev)
        }
    }
}