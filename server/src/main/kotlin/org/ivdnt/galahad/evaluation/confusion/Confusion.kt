...
open class Confusion(private val truncate: Boolean = true, val annotation: AnnotationType): CsvSampleExporter {
    ...
    fun add(termComp: TermComparison) {
        add(
            termComp.hypoTerm.annotationToGroupHeadOrDefault(annotation) ?: Term.missingName(annotation),
            termComp.refTerm.annotationToGroupHeadOrDefault(annotation) ?: Term.missingName(annotation),
            termComp
        )
    }
