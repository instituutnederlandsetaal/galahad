...
class DocumentDistribution(@JsonValue val typeTokens: Map<Annotation, List<TypeToken>>) {
    companion object {
        private val ANNOTATIONS =
            arrayOf(Annotation.POS, Annotation.UPOS, Annotation.NER, Annotation.DEPREL)

        fun create(layer: Layer): DocumentDistribution =
            DocumentDistribution(
                buildMap<Annotation, MutableMap<Pair<String, String>, MutableMap<String, Int>>> {
                        // We're going to construct a map for each annotation,
                        // with key [lemma; that annotation head] and [tokens; their counts] as
                        // values.
                        ...
                        layer.terms.forEach { t ->
                            ...
                            // values
                            t.annotations
                                .filter { it.key in ANNOTATIONS }
                                .forEach { (annotation, _) ->
                                    ...
                                    val pair =
                                        t.annotationOrMissing(Annotation.LEMMA) to
                                            t.annotationHeadOrMissing(annotation)
                                    val tokens = mutableMapOf(t.token to 1)
                                    merge(annotation, mutableMapOf(pair to tokens)) {
                                        oldTypeTokens,
                                        newTypeTokens ->
                    ...
                    .mapValues { (_, group) ->
                        ...
                        group
                            .map { (k, v) ->
                                TypeToken(lemma = k.first, group = k.second, tokens = v)
                            }
