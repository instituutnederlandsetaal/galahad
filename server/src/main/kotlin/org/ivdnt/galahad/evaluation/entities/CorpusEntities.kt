...
class CorpusEntities(val jobs: Map<String, JobEntities>, val stddev: JobsEntitiesStddev) {
    ...
    class DocumentEntitiesStddev(val stddev: Map<String, Double>, val average: Double)
    ...
    companion object {
        fun create(corpus: Corpus, jobsEvals: CorpusEvaluation): CorpusEntities {
            ...
            val jobEntities: Map<String, JobEntities> =
                jobs
                    .associateWith { job -> jobsEvals.createOrThrow(job).entities }
                ...
                corpus.documents
                    ...
                    .associateWith { doc ->
                        ...
                        val counts: Map<String, List<Int>> = buildMap {
                            val summaries =
                                jobEntities.values.map { it.documents[doc]?.summary ?: emptyMap() }
                            summaries.forEach { it.keys.forEach { putIfAbsent(it, emptyList()) } }
                        ...
                        DocumentEntitiesStddev(
                            stddev,
                            stddev.values.average().takeIf { !it.isNaN() } ?: 0.0,
                        )
                    }

            val labelAvg: Map<String, Double> =
                docstddevs.values
                    .flatMap { it.stddev.keys }
                    .distinct()
                    .associateWith { label ->
        ...
        private fun getLabels(entities: CorpusEntities): List<String> =
            entities.stddev.stddev.keys.sorted()
        ...
        private fun getDocs(entities: CorpusEntities): List<String> =
            entities.stddev.documents.keys.sorted()
        ...
        private fun getHeader(entities: CorpusEntities): MutableList<String> {
            ...
            getJobs(entities).forEach { name ->
                getLabels(entities.jobs[name]!!).forEach { label -> header.add("$name $label") }
                header.add("$name total")
            }
            getLabels(entities).forEach { label -> header.add("$label std") }
            header.add("stdavg")
            return header
        }
    }
