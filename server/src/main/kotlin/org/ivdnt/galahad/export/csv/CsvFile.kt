...
class CsvFile(path: File) : File(path.toURI()) {
    ...
    companion object {
        // Alternatively we could check for forbidden characters first, and the wrap/replace only
        // when
        // necessary.
        // However, this works and gives a consistent result
        private fun csvEscape(s: String): String = "\"${s.replace("\"", "\"\"")}\""

        // BOM forces Excel to read UTF16LE. Needed for e.g. 'ü'.
