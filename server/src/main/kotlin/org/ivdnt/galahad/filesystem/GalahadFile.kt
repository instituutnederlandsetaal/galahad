...
abstract class GalahadFolder<ReadType : GalahadFile, CreateType>(
    ...
    inline fun <reified ReadType> readAll(): Set<ReadType> =
        dir.list()?.map { readOrThrow<ReadType>(it) }?.toSet() ?: setOf()

    inline fun <reified ReadType> readOrThrow(key: String): ReadType =
        readOrNull(key) ?: throw Exception("Failed to read $key")
